...
far_detector_spectrum_mu = np.loadtxt('data/output_nu_mu_bar.txt') #One extracts also the output
Emu_FD_article, specmu_FD_article = np.array([i[0] for i in far_detector_spectrum_mu]), np.array([i[1] for i in far_detector_spectrum_mu])

plt.plot( Emu_FD_article, specmu_FD_article )
plt.show()
plt.close()

fd_spectrum_mu_noise = np.loadtxt('data/output_nu_mu_bar_noise.txt') #Noise on the energy distribution of the Nvmu
...
log10E, sigma_o_E_numubar_data = np.array([i[0] for i in sigma_o_E]), np.array([i[5] for i in sigma_o_E])*1e-38
E = pow( 10 , log10E)
sigma_o_E_numubar_interp = interp1d(E, sigma_o_E_numubar_data, kind='cubic')
sigma_numubar = np.array(sigma_o_E_numubar_interp(E_lisseND)) * E_lisseND 
...
sigma_FD = m_Ar_FD * Na/M_Ar * N_nuc_per_Ar * sigma_numubar #cross section of the reaction ( nu_e + n -> e- + p ) in the FD (in cm^2), same for nu_mu than for nu_e
...
plt.plot(E, m_Ar_FD * Na/M_Ar * N_nuc_per_Ar * sigma_o_E_numubar_data * E, 'x', label='$\sigma_{\u03BD_e}^{Ar}$  data')
...
"""
L_delta_m32 = np.linspace (2.32e-03,2.7e-03,150) 
L_theta23 = np.linspace(0,np.pi/2,150)
...
"""
L_accuracy = np.linspace(0,30,70)
L_N_nue = []
...
L_bias = np.linspace(-15,10,80)
L_Reco= np.linspace(8,25,80)
LL2_N_nue = []
L2_khi2 = []
